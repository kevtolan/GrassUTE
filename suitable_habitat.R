...
# st_write(VT_hay_pasture_22_parcels,"~/R/Grasslab hab/VT_hay_pasture_22.shp")
...
mapview(VT_hay_pasture_16_parcels)

# st_write(VT_hay_pasture_16_soils,"~/R/Grasslab hab/VT_hay_pasture_16_soils.geojson")

hay_16_union <- st_union(st_geometry(VT_hay_pasture_16_parcels))
hay_22_union <- st_union(st_geometry(VT_hay_pasture_22))

VT_lost_gland <- st_difference(VT_hay_pasture_16_parcels, hay_22_union) %>%
  mutate(Status = "lost")

VT_gained_gland <- st_difference(VT_hay_pasture_22_parcels, hay_16_union) %>%
